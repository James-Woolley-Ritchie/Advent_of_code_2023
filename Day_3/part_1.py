...
with open("Day_3\\input.txt", "r") as input:
    ...
    for line_index in range(len(schematic)):
        for character_index in range(len(schematic[line_index])):
            if schematic[line_index][character_index].lower() not in "abcdefghijklmnopqrstuvwxyz.1234567890":
                special_character_indexes.append([line_index, character_index]) # Find the indexes of all the special characters.
            ...
            for i in range(3):
                for index in indexes_to_check:
                    if number_and_indexes[i + 1] == index:
                        total += int(number_and_indexes[0])
                        valid_number = True
                        break
                if valid_number:
                    break
